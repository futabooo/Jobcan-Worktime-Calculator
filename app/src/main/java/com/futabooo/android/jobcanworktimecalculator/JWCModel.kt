...
class Attendance constructor(private val document: Document) {
  ...
  val date: String = document.select("table.left:contains(ユーザー情報) tbody tr:eq(0) td")
      .text()
  val requiredWorkDay: String = document.select("table.left:contains(ユーザー情報) tbody tr:eq(3) td")
      .text()
      .removeSuffix("日")
      .trim()
  val actualWorkDay: String = document.select("table.right:contains(基本項目) tbody tr:eq(0) td")
      .text()
  val actualWorkTime: String = document.select("table.left:contains(労働時間) tbody tr:eq(0) td")
      .text()
      .replace(":", ".")
  ...
  fun requiredWorkTime(): String {
    return String.format("%1.2f", requiredWorkDay.toInt() * 8.0f)
  }

  fun savingTime(): String {
    val daysLeft = requiredWorkDay.toInt() - actualWorkDay.toInt()
    val actual = actualWorkTime.replace(":", ".")
        .toFloat()
